...
extern const char FRF_SC_CARTRIDGE_ID[] = "FRF_FRODOSC_CRT_MAPPING_1.0.15";
...
{
	/* FRF 1.0.15: cartridge I/O must take priority over REU/open bus. */
...
{
	/* FRF 1.0.15: cycle-exact CPU cartridge mapping. */
...
{
	/* FRF 1.0.15: cycle-exact cartridge bank/control writes. */
	if (FRFEasyFlashIsLoaded() && io_in) {
		if (adr >= 0xde00 && adr <= 0xdeff) {
...
{
	/* Writes beneath ROML still reach C64 RAM and the mapper shadow. */
	if (FRFEasyFlashIsLoaded() && adr >= 0x8000 && adr <= 0x9fff &&
...
void MOS6510::Reset(void)
{
	...
	/* Restore the inserted cartridge after Frodo clears CBM80. */
	if (FRFEasyFlashIsLoaded())
