...
private:
	...
	/* FRF_RGBFAST/FIX44 late-line redraw and junction helpers. */
	void frf_vic_mark_sprite_line_dirty(void);
	bool frf_vic_arm_same_raster_handover(int snum);
	void frf_vic_reconcile_next_sprite(int snum, bool include_active);
	void frf_vic_reconcile_drawn_sprite(int snum, bool include_active);
	void frf_vic_apply_horizontal_border(uint8 *chunky_ptr);
	void frf_vic_flush_late_sprite_line(void);
	...
	/* FRF_RGBFAST/FIX44 late-line redraw state (not serialized). */
	uint8 frf_line_background[0x180];
	uint16 frf_line_mc[8];
	uint8 *frf_line_ptr;
	uint8 frf_line_sprite_on;
	uint8 frf_line_clx_spr;
	uint8 frf_line_clx_bgr;
	bool frf_line_valid;
	bool frf_line_dirty;
	uint8 frf_same_raster_handover;
	uint8 frf_force_mc_advance;
