...
Prefs::Prefs()
{
	...
	StartupDisplay = FRF_STARTUP_DISPLAY_WINDOWED; /* FRF_GUI_OUTPUT_PREFS_1.0.31 */
...
bool Prefs::operator==(const Prefs &rhs) const
{
	return (1
		...
		&& StartupDisplay == rhs.StartupDisplay
...
void Prefs::Check(void)
{
	...
	if (StartupDisplay < FRF_STARTUP_DISPLAY_WINDOWED ||
	    StartupDisplay > FRF_STARTUP_DISPLAY_ASK)
		StartupDisplay = FRF_STARTUP_DISPLAY_WINDOWED;
			...
			if (sscanf(line, "%s = %s\n", keyword, value) == 2) {
				if (!strcmp(keyword, "NormalCycles"))
					...
					if (!strcmp(value, "128K"))
						...
						REUSize = REU_512K;
                                else if (!strcmp(value, "1M"))
                                        REUSize = REU_1M;
                                else if (!strcmp(value, "2M"))
                                        REUSize = REU_2M;
                                else if (!strcmp(value, "4M"))
                                        REUSize = REU_4M;
                                else if (!strcmp(value, "8M"))
                                        REUSize = REU_8M;
				...
				else if (!strcmp(keyword, "StartupDisplay")) {
					if (!strcmp(value, "RTG"))
						StartupDisplay = FRF_STARTUP_DISPLAY_RTG;
					else if (!strcmp(value, "RGB") || !strcmp(value, "PAL") || !strcmp(value, "PAL320"))
						StartupDisplay = FRF_STARTUP_DISPLAY_RGB;
					else if (!strcmp(value, "ASK") || !strcmp(value, "REQUESTER"))
						StartupDisplay = FRF_STARTUP_DISPLAY_ASK;
					else
						StartupDisplay = FRF_STARTUP_DISPLAY_WINDOWED;
...
bool Prefs::Save(char *filename)
{
	...
	if ((file = fopen(filename, "w")) != NULL) {
		...
		fprintf(file, "StartupDisplay = ");
		switch (StartupDisplay) {
			case FRF_STARTUP_DISPLAY_RTG:
				fprintf(file, "RTG\n");
				break;
			case FRF_STARTUP_DISPLAY_RGB:
				fprintf(file, "RGB\n");
				break;
			case FRF_STARTUP_DISPLAY_ASK:
				fprintf(file, "ASK\n");
				break;
			default:
				fprintf(file, "WINDOWED\n");
				break;
		}
