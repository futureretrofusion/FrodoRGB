...
void MOS6510::Reset(void)
{
	 ...
	 *
	 * A hardware reset must not eject the inserted CRT. Normal 8K and
	 * Magic Desk cartridges boot through the KERNAL CBM80 probe at $8004.
	 * Frodo deliberately clears that signature above, so restore the
	 * cartridge's power-on bank and visible ROM windows before continuing.
	 * EasyFlash also returns to its reset bank/mode through this path.
	 */
	if (FRFEasyFlashIsLoaded())
...
void MOS6510::illegal_jump(uint16 at, uint16 to)
{
        /*
         * CRT_FORCE11:
         * allow EasyFlash IO2 jump.
         * EasyFlash deliberately executes helper code from $DF00-$DFFF.
         */
        if (FRFEasyFlashAllowIO2Execute((unsigned short)(at),
                                        (unsigned short)(to)))
                return;
