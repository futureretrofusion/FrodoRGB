...
#ifndef FRF_VIC_PHASE_BIAS_NORMAL
#define FRF_VIC_PHASE_BIAS_NORMAL 1
#endif
#ifndef FRF_VIC_PHASE_BIAS_EXPANDED
#define FRF_VIC_PHASE_BIAS_EXPANDED FRF_VIC_PHASE_BIAS_NORMAL
#endif
#define FRF_VIC_STRINGIFY_2(x) #x
#define FRF_VIC_STRINGIFY(x) FRF_VIC_STRINGIFY_2(x)
extern const char FRF_VIC_TIMING_ID[] =
    "FRF_VIC_FIX46_PHASE_SWEEP N=" FRF_VIC_STRINGIFY(FRF_VIC_PHASE_BIAS_NORMAL)
    " E=" FRF_VIC_STRINGIFY(FRF_VIC_PHASE_BIAS_EXPANDED);
/*
 * FIX46 keeps the improved FIX45 next-line-only model but makes the vertical
 * catch-up bias explicit. Separate normal and Y-expanded values allow target
 * testing without changing any other VIC, framebuffer or cartridge logic.
 */
static inline int frf_vic_late_sprite_row(int diff, bool expanded)
{
    const int bias = expanded ? FRF_VIC_PHASE_BIAS_EXPANDED
                              : FRF_VIC_PHASE_BIAS_NORMAL;
    int phase = diff - bias;
    if (phase < 0)
        phase = 0;
    return expanded ? (phase >> 1) : phase;
}
...
void MOS6569::WriteRegister(uint16 adr, uint8 byte)
{
	switch (adr) {
		...
		case 0x09: case 0x0b: case 0x0d: case 0x0f:{
			...
			/*
			 * FRF_VIC_FIX42_LATE_SPRITE_Y_COMPARE
			 *
			 * Frodo's fast VIC is line based: it renders the whole raster
			 * line before the 6510 gets to run the IRQ handler for that
			 * line. Raster-multiplexed games can therefore write a sprite
			 * Y value just after our once-per-line Y compare has already
			 * happened. On real VIC-II hardware, a same-line/near-line
			 * sprite Y change can still arm the sprite DMA/display logic
			 * at the right time.
			 *
			 * If a sprite is enabled and its new Y value is at or just
			 * behind the current raster, catch the line-mode core up by
			 * arming the sprite and setting the sprite data counter to the
			 * row that should currently be visible. This specifically helps
			 * heavy sprite multiplexers such as SSF2T where the top halves
			 * of multiplexed sprites were missing.
			 */
			if (me & sbit) {
				int diff = (int)((uint8)(raster_y - byte));
				int max_lines = (mye & sbit) ? 42 : 21;

				if (diff >= 0 && diff < max_lines) {
					/* FRF_VIC_FIX46_PHASE_SWEEP: map the next rendered raster
					 * through the selected target-test Y phase bias. */
					int row = frf_vic_late_sprite_row(diff, (mye & sbit) != 0);
					mc[snum] = (uint16)(row * 3);
					sprite_on |= sbit;
				} else if (diff >= max_lines && diff < 0x80) {
					/* New Y is behind us and outside the sprite height.
					 * Stop showing stale rows from the previous multiplex use. */
					mc[snum] = 63;
					sprite_on &= ~sbit;
				}
			}
		...
		case 0x15:{	// Sprite enable
			...
			/* FRF_VIC_FIX42_ENABLE_CATCHUP:
			 * If sprite enable is turned on after the line compare already
			 * ran for this raster, arm it when its Y position is current. */
			uint8 newly_on = (uint8)(me & ~old_me);
			if (newly_on) {
				for (int snum=0, sbit=1; snum<8; snum++, sbit<<=1) {
					if (newly_on & sbit) {
						int diff = (int)((uint8)(raster_y - my[snum]));
						int max_lines = (mye & sbit) ? 42 : 21;
						if (diff >= 0 && diff < max_lines) {
							/* Keep late-enable catch-up on the same Y phase model. */
							int row = frf_vic_late_sprite_row(diff, (mye & sbit) != 0);
							mc[snum] = (uint16)(row * 3);
							sprite_on |= sbit;
						}
