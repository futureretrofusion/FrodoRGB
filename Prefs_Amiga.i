...
/*
 *  Prefs_Amiga.i - Global preferences, Amiga specific stuff
 *
...
#ifndef FRODO_PREFS_OPEN_DEBUG_LOG
#define FRODO_PREFS_OPEN_DEBUG_LOG
#include <stdio.h>

static void FrodoPrefsOpenLog(const char *msg)
{
#if FRF_ENABLE_DIAGNOSTIC_LOGS
	FILE *f = FRFOpenDiagnosticLog("PROGDIR:frodo-prefs-open.log", "frodo-prefs-open.log", "a");
	if (f) {
		fprintf(f, "%s\n", msg);
		fclose(f);
	}
...
bool Prefs::ShowEditor(bool startup, char *prefs_name)
{
	...
	// Open prefs window
	FrodoPrefsOpenLog("before SetupScreen");
	if (!SetupScreen()) {
		FrodoPrefsOpenLog("after SetupScreen");
		FrodoPrefsOpenLog("before OpenPrefsWindow");
		if (!OpenPrefsWindow()) {
			FrodoPrefsOpenLog("after OpenPrefsWindow");
		...
		FrodoPrefsOpenLog("before ClosePrefsWindow");
		ClosePrefsWindow();
		FrodoPrefsOpenLog("after ClosePrefsWindow");
	}
	FrodoPrefsOpenLog("before CloseDownScreen");
	CloseDownScreen();
	FrodoPrefsOpenLog("after CloseDownScreen");
...
static void set_values(void)
{
	...
	GT_SetGadgetAttrs(PrefsGadgets[GDX_StartupDisplay], PrefsWnd, NULL, GTCY_Active, prefs->StartupDisplay, TAG_DONE); /* FRF_GUI_OUTPUT_PREFS_1.0.31 */
...
int StartupDisplayClicked(void)
{
        prefs->StartupDisplay = PrefsMsg.Code;
}
