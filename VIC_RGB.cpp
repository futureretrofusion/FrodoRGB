...
/*
 * FRF_RGBFAST_VIC_1.0.26
 *
 * Dedicated fast native-RGB VIC. This keeps the line-based renderer and the
 * FIX44 completed-raster sprite-junction repair, then presents the final line
 * only after the CPU timeslice has applied its raster-IRQ sprite writes.
 */
/* FRF_VIC_FIX42: line-mode sprite multiplex catch-up for late Y writes. */
extern const char FRF_VIC_TIMING_ID[] = "FRF_VIC_FIX44";
/*
 * FRF_VIC_FIX44: same-raster sprite-multiplex junction repair.
 *
 * FIX43 can redraw a line after its CPU timeslice, but a special handover still
 * remained: an IRQ can rewrite a previously inactive hardware sprite to the
 * raster that has already been drawn. In that case the saved line contains only
 * background, FIX43 phases the new sprite for the following raster, and one
 * clean background line appears through the multiplexed object.
 *
 * FIX44 detects only that missing-overlay case. It inserts sprite row 0 into the
 * completed raster and phases mc[] for the following raster. Non-expanded
 * sprites continue at row 1; Y-expanded sprites repeat row 0 exactly once and
 * use a one-shot counter advance after the next line. Existing active sprites
 * are not restarted, avoiding broad changes to ordinary sprite rendering.
 */
...
// First and last generated pixel line for the dedicated RGB-fast core.
// Timing still runs for all 312 PAL rasters; only the central 320x200 display
// window is rendered into the chunky line buffer.
const unsigned FIRST_DISP_LINE = 0x33;
const unsigned LAST_DISP_LINE = 0xfa;
...
/* FRF_VIC_FIX43 late-line redraw state. */
static uint8 frf_line_background[DISPLAY_X];
static uint16 frf_line_mc[8];
static uint8 *frf_line_ptr;
static uint8 frf_line_sprite_on;
static uint8 frf_line_clx_spr;
static uint8 frf_line_clx_bgr;
static bool frf_line_valid;
static bool frf_line_dirty;
static uint8 frf_same_raster_handover;
static uint8 frf_force_mc_advance;
...
#endif
{
	...
	frf_line_ptr = NULL;
	frf_line_sprite_on = 0;
	frf_line_clx_spr = frf_line_clx_bgr = 0;
	frf_line_valid = false;
	frf_line_dirty = false;
	frf_same_raster_handover = 0;
	frf_force_mc_advance = 0;
...
void MOS6569::SetState(MOS6569State *vd)
{
	...
	/* A loaded snapshot has no pending host-side late-line redraw. */
	frf_line_ptr = NULL;
	frf_line_sprite_on = 0;
	frf_line_valid = false;
	frf_line_dirty = false;
...
/*
 * FRF_VIC_FIX44 helpers
 */
#ifdef GLOBAL_VARS
static inline void frf_vic_mark_sprite_line_dirty(void)
#else
inline void MOS6569::frf_vic_mark_sprite_line_dirty(void)
#endif
{
	if (frf_line_valid)
		frf_line_dirty = true;
...
#ifdef GLOBAL_VARS
static inline bool frf_vic_arm_same_raster_handover(int snum)
#else
inline bool MOS6569::frf_vic_arm_same_raster_handover(int snum)
#endif
{
	const int sbit = 1 << snum;

	/* FIX44 targets only the visible hole: the hardware sprite was absent from
	 * the completed raster, is enabled now, and its new Y equals that raster. */
	if (!frf_line_valid || !(me & sbit) || (frf_line_sprite_on & sbit))
		return false;
	if ((uint8)(raster_y - my[snum]) != 0)
		return false;

	frf_line_mc[snum] = 0;
	frf_line_sprite_on |= (uint8)sbit;
	sprite_on |= (uint8)sbit;
	frf_same_raster_handover |= (uint8)sbit;

	if (mye & sbit) {
		/* Row 0 is painted now and repeated on the next raster. Force the
		 * counter to advance after that repeat, because the original Y parity
		 * test assumes row 0 first appeared one raster later. */
		mc[snum] = 0;
		frf_force_mc_advance |= (uint8)sbit;
	} else {
		/* Row 0 is painted into the completed raster; the next line is row 1. */
		mc[snum] = 3;
		frf_force_mc_advance &= (uint8)~sbit;
	}

	frf_vic_mark_sprite_line_dirty();
	return true;
}

#ifdef GLOBAL_VARS
static inline void frf_vic_reconcile_next_sprite(int snum, bool include_active)
#else
inline void MOS6569::frf_vic_reconcile_next_sprite(int snum, bool include_active)
#endif
{
	const int sbit = 1 << snum;
	if (!(me & sbit) && !(include_active && (sprite_on & sbit)))
		return;

	const int diff = (int)((uint8)(raster_y - my[snum]));
	const int max_lines = (mye & sbit) ? 42 : 21;

	if (diff >= 0 && diff < max_lines) {
		const int row = (mye & sbit) ? (diff >> 1) : diff;
		mc[snum] = (uint16)(row * 3);
		sprite_on |= sbit;
	} else if (diff >= max_lines && diff < 0x80) {
		mc[snum] = 63;
		sprite_on &= ~sbit;
	}
}
...
#ifdef GLOBAL_VARS
static inline void frf_vic_reconcile_drawn_sprite(int snum, bool include_active)
#else
inline void MOS6569::frf_vic_reconcile_drawn_sprite(int snum, bool include_active)
#endif
{
	if (!frf_line_valid)
		return;
	...
	const int sbit = 1 << snum;
	if (!(me & sbit) && !(include_active && (frf_line_sprite_on & sbit)))
		return;

	/*
	 * A sprite whose Y compare was on the previous raster should have row 0
	 * on the line that has already been rendered. Keep this draw-phase
	 * counter separate from mc[], which is already phased for the next line.
	 */
	const int diff = (int)((uint8)(raster_y - my[snum]));
	const int max_lines = (mye & sbit) ? 42 : 21;

	if (diff > 0 && diff <= max_lines) {
		const int row = (mye & sbit) ? ((diff - 1) >> 1) : (diff - 1);
		frf_line_mc[snum] = (uint16)(row * 3);
		frf_line_sprite_on |= sbit;
	} else if (diff > max_lines && diff < 0x80) {
		frf_line_mc[snum] = 63;
		frf_line_sprite_on &= ~sbit;
	}
...
void MOS6569::WriteRegister(uint16 adr, uint8 byte)
{
	switch (adr) {
		...
		case 0x01: case 0x03: case 0x05: case 0x07:
		case 0x09: case 0x0b: case 0x0d: case 0x0f:{
			const int snum = adr >> 1;
			const int sbit = 1 << snum;

			/* Multiple writes to the same Y register in one IRQ must leave only
			 * the final handover state. Undo an earlier provisional repair. */
			if (frf_same_raster_handover & sbit) {
				frf_same_raster_handover &= (uint8)~sbit;
				frf_force_mc_advance &= (uint8)~sbit;
				frf_line_sprite_on &= (uint8)~sbit;
				frf_line_mc[snum] = 63;
			...
			/* FIX44 first handles the exact same-raster junction that otherwise
			 * leaves background between two multiplex uses. Other late Y writes
			 * retain the broader FIX42/FIX43 phase reconciliation. */
			if (!frf_vic_arm_same_raster_handover(snum)) {
				frf_vic_reconcile_next_sprite(snum, false);
				frf_vic_reconcile_drawn_sprite(snum, false);
				frf_vic_mark_sprite_line_dirty();
			}
			break;
		}
		...
		case 0x15:{	// Sprite enable
			const uint8 old_me = me;
			me = byte;

			/* If a provisional same-raster repair is disabled again in the same
			 * CPU timeslice, remove it so only the final D015 state is painted. */
			const uint8 newly_off = (uint8)(old_me & ~me);
			for (int snum=0, sbit=1; snum<8; snum++, sbit<<=1) {
				if ((newly_off & sbit) && (frf_same_raster_handover & sbit)) {
					frf_same_raster_handover &= (uint8)~sbit;
					frf_force_mc_advance &= (uint8)~sbit;
					frf_line_sprite_on &= (uint8)~sbit;
					frf_line_mc[snum] = 63;
			...
			/* Newly enabled sprites may have missed both the previous raster's
			 * draw point and the once-per-line Y comparison. Reconcile both
			 * phases. Active sprites are deliberately not killed immediately
			 * when their enable bit is cleared; that matches the old DMA model. */
			const uint8 newly_on = (uint8)(me & ~old_me);
			for (int snum=0, sbit=1; snum<8; snum++, sbit<<=1) {
				if (newly_on & sbit) {
					if (!frf_vic_arm_same_raster_handover(snum)) {
						frf_vic_reconcile_next_sprite(snum, false);
		...
		case 0x17:{	// Sprite Y expansion
			const uint8 old_mye = mye;
			mye = byte;

			/* FIX43_D017_REPHASE: the old line core changed the expansion
			 * register without updating mc[]. If an IRQ wrote Y first and D017
			 * second, the row phase depended on write order and produced seams. */
			if (old_mye != mye) {
				for (int snum=0, sbit=1; snum<8; snum++, sbit<<=1) {
					if (frf_same_raster_handover & sbit) {
						/* Keep the repaired current row at zero, but rephase the
						 * following raster if D017 is written after the Y register. */
						frf_line_mc[snum] = 0;
						if (mye & sbit) {
							mc[snum] = 0;
							frf_force_mc_advance |= (uint8)sbit;
						...
						frf_vic_reconcile_next_sprite(snum, true);
						frf_vic_reconcile_drawn_sprite(snum, true);
					}
				}
				frf_vic_mark_sprite_line_dirty();
			}
		...
		case 0x18:	// Memory pointers
			...
			/* Sprite pointers live in the matrix page; redraw with the final
			 * page selected by the raster IRQ. */
			frf_vic_mark_sprite_line_dirty();
			break;
		...
		case 0x25:
			mm0_color = colors[mm0 = byte];
			frf_vic_mark_sprite_line_dirty();
			break;
		case 0x26:
			mm1_color = colors[mm1 = byte];
			frf_vic_mark_sprite_line_dirty();
			break;
...
/*
 * FRF_VIC_FIX43: repaint the previous raster once after all of its CPU-side
 * sprite writes have completed. The graphics/foreground mask still belong to
 * that raster here; EmulateLine() calls this before constructing the next one.
 */
#ifdef GLOBAL_VARS
static inline void frf_vic_apply_horizontal_border(uint8 *chunky_ptr)
#else
inline void MOS6569::frf_vic_apply_horizontal_border(uint8 *chunky_ptr)
#endif
{
...
#ifdef GLOBAL_VARS
static inline void frf_vic_flush_late_sprite_line(void)
#else
inline void MOS6569::frf_vic_flush_late_sprite_line(void)
#endif
{
	if (!frf_line_valid || frf_line_ptr == NULL)
		return;
	...
	if (frf_line_dirty) {
		...
		for (int i=0; i<8; i++)
			next_mc[i] = mc[i];
		const uint8 next_sprite_on = sprite_on;

		memcpy(frf_line_ptr, frf_line_background, DISPLAY_X);
		clx_spr = frf_line_clx_spr;
		clx_bgr = frf_line_clx_bgr;
		for (int i=0; i<8; i++)
			mc[i] = frf_line_mc[i];
		sprite_on = frf_line_sprite_on;

		if (sprite_on && ThePrefs.SpritesOn) {
			uint32 *lp = (uint32 *)spr_coll_buf - 1;
			for (int i=0; i<DISPLAY_X/4; i++)
				*++lp = 0;
			el_sprites(frf_line_ptr);
		}

		frf_vic_apply_horizontal_border(frf_line_ptr);

		for (int i=0; i<8; i++)
			mc[i] = next_mc[i];
		sprite_on = next_sprite_on;
	}

#ifdef AMIGA
#ifdef FRF_RGBFAST
	/* Present the final post-IRQ raster, not the provisional pre-CPU line. */
	the_display->UpdateScanline(
		frf_line_ptr,
		(int)((frf_line_ptr - the_display->BitmapBase()) / xmod));
#endif
#endif
...
#endif
{
	...
	for (i=0, j=1; i<8; i++, j<<=1, mcp++, myp++) {
		...
		/* FIX44 expanded same-raster handover: row 0 was inserted into the
		 * completed raster and repeated on this raster. Advance once now so
		 * the next raster starts row 1 rather than showing row 0 three times. */
		if (frf_force_mc_advance & j) {
			frf_force_mc_advance &= (uint8)~j;
			if (*mcp != 63) {
				/* Counter-phase correction only: do not charge another DMA
				 * slot on the following line for a row already fetched. */
				*mcp += 3;
				if (*mcp == 63)
					spron &= (uint8)~j;
			}
...
{
	/* Finish the previous raster with the final sprite register state written
	 * by its CPU timeslice before fore_mask_buf is reused for this line. */
	frf_vic_flush_late_sprite_line();
	frf_line_valid = false;
	frf_line_dirty = false;
	...
	if (raster >= FIRST_DISP_LINE && raster <= LAST_DISP_LINE) {
		...
		if (!border_on) {
			...
			/* FIX43 draw checkpoint: save the sprite-free line and the exact
			 * counters used for this raster. CPU writes can then repair this
			 * line once, without accumulating repeated sprite paint. */
			memcpy(frf_line_background, chunky_ptr, DISPLAY_X);
			frf_line_ptr = chunky_ptr;
			for (int i=0; i<8; i++)
				frf_line_mc[i] = mc[i];
			frf_line_sprite_on = sprite_on;
			frf_line_clx_spr = clx_spr;
			frf_line_clx_bgr = clx_bgr;
			frf_line_valid = true;
			frf_line_dirty = false;
